...
def init_worker(max_unchanged_words_p, beta_p, ignore_whitespace_casing_p, verbose_p, very_verbose_p, skip_lines, use_limit_p):
    global max_unchanged_words, beta, ignore_whitespace_casing, verbose, very_verbose, g_skip_lines, use_limit
...
def wrapper_func_m2scorer(tuple_items) -> Tuple[int, int, int]:
    ...
    stat_correct, stat_proposed, stat_gold = batch_multi_pre_rec_f1_part(
        ...
        source_sentence, 
        gold_edit,
        max_unchanged_words, beta, ignore_whitespace_casing, verbose, very_verbose, use_limit)
...
def main(config_filename: str):
    ...
    SKIP_LINES = config.get('skip_lines', False)

    USE_LIMIT = config.get('use_limit', True)
    if USE_LIMIT is None:
        print("LIMIT TIMEOUT is NOT used...")
    else:
        print("LIMIT TIMEOUT is used...")
    ...
    def compute_metrics_m2scorer(tokenized_predicted_sentences, source_sentences, gold_edits):
        ...
        with Pool(processes=NUM_EVAL_PROCESSES * 2, initializer=init_worker, initargs=(MAX_UNCHANGED_WORDS, BETA, IGNORE_WHITESPACE_CASING, VERBOSE, VERY_VERBOSE, SKIP_LINES, USE_LIMIT,)) as pool:
